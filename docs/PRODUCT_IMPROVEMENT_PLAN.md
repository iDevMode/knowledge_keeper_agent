...
- ✅ **Entity memory (2.1)** — the per-answer Haiku call now returns risk flags *and* named entities (one structured call, no extra latency); entities accumulate in graph state, unprobed ones are surfaced to the primary model so questions reference them by name, and an **entity sweep** phase before closing asks targeted questions about high-importance entities no block question reached.
- ✅ **Information-gain follow-ups (2.2)** — the follow-up classifier sees what's already captured for the block and rates expected gain (`none/low/medium/high`); low-gain follow-ups are suppressed, refusals are never chased, and over the turn budget only high-gain follow-ups run.
- ✅ **Fatigue management (2.3)** — a per-session question budget is computed from the block plan; over budget, light blocks shrink to one question, and at a hard cap the interview jumps to a graceful close. Micro-summaries at block transitions state honest progress ("section 3 of 7, ~40%"), and the same numbers are exposed via `GET /api/sessions/{id}/status` and rendered as a progress bar in the employee UI. (Multi-sitting resume shipped in Phase 1.)
- ✅ **Eval harness (4.5, pulled forward)** — `evals/` package with synthetic personas carrying planted facts (direct / hidden-behind-vagueness / entity-name-drop), a headless runner that drives the real compiled graph, and a scorecard: planted-knowledge recall by reveal type, risk recall, follow-up precision, budget adherence, coverage, single-question compliance. Scripted mode uses deterministic fakes (zero API calls — runs in CI via `tests/test_evals.py`, including tests that *break* a capability and assert the scorecard goes red); `python -m evals.run_eval --live` runs the same scorecard with real models. See `docs/EVALS.md`.
- 🔲 Remaining: Stage 1 context ingestion + editable profile review form (2.5) · SSE streaming + prompt caching (4.4) — a transport/UI change independent of the interview logic above.
