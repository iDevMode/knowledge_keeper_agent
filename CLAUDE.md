...
Any Postgres will do — Railway, Supabase, anything — but **the connection must not be transaction-pooled**. `PostgresSessionLocks` holds a session-scoped `pg_advisory_lock` on one connection for the duration of a graph run. A transaction-mode pooler (Supabase's Supavisor on port 6543, PgBouncer likewise) routes consecutive statements to different backends, so the lock is taken on one and released on another and stops serialising anything — no error, no log line, just the cross-worker checkpoint interleaving the lock exists to prevent. Use a session-mode pooler (port 5432) or a direct connection. `uses_transaction_pooler()` in `config/settings.py` checks the port at startup: a warning on one worker, fatal above that. If you ever want transaction pooling for the store and checkpointer pools, the lock pool needs its own session-mode URL first.
